...
def record_mode(filename, topic):
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', choices=['record', 'plot', 'test'], help="Mode: record (ROS->CSV), plot (CSV->PNG), test (Gen Dummy CSV)")
    ...
    if args.mode == 'record':
        record_mode(args.file, args.topic)
    elif args.mode == 'plot':
        plot_mode(args.file)
    elif args.mode == 'test':
...
# # 1. 录制数据
# python monitor_joints.py record -f my_experiment.csv
# python monitor_joints.py record -f test_motion.csv
# # 2. 录制结束后（Ctrl+C），生成图表
# python monitor_joints.py plot -f my_experiment.csv
# # 绘制位置
# python monitor_joints.py plot -f test_motion.csv

# # 绘制速度
# python monitor_joints.py plot -f test_motion_vel.csv

# # 绘制加速度
# python monitor_joints.py plot -f test_motion_acc.csv