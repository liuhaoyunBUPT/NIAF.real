...
class NormalizeActions(object):
    """Normalize actions to [-1, 1] range using min and max values."""
    ...
    def __call__(self, action):
        # 2 * (x - min) / (max - min) - 1
        return 2 * (action - self.min_action) / (self.max_action - self.min_action) - 1

    def inverse(self, normalized_action):
        # (y + 1) / 2 * (max - min) + min
        return (normalized_action + 1) / 2 * (self.max_action - self.min_action) + self.min_action

    def __repr__(self):
        return self.__class__.__name__ + f"(min_action={self.min_action}, max_action={self.max_action})"
