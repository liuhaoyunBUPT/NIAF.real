...
def get_all_checkpoints(experiment_folder: Path) -> List:
    """
    返回 saved_models 下（包含子目录）所有 .ckpt 文件，按修改时间升序
    """
    checkpoint_folder = experiment_folder / "saved_models"
    if not checkpoint_folder.is_dir():
        return []
    ckpts = list(checkpoint_folder.rglob("*.ckpt"))
...
# =====================================================================
# Checkpoint loading utilities (migrated from evaluation/utils.py)
# =====================================================================

def load_class(name: str):
    """Dynamically load a class by its fully-qualified name."""
    import importlib
...
def get_checkpoint_i_from_dir(dir: Path, i: int = -1) -> Path:
    """Get the i-th checkpoint from a directory. -1 returns 'last.ckpt'."""
...
def get_config_from_dir(dir: Union[Path, str]):
    """Load hydra config.yaml from a training run directory."""
...
def load_pl_module_from_checkpoint(
    ...
    """Load a PyTorch Lightning module from a checkpoint file or directory.
    
    Args:
        filepath: Path to checkpoint file (.ckpt) or training run directory.
        epoch: Which epoch checkpoint to load (-1 = last).
        overwrite_cfg: Config overrides to apply when loading.
        use_ema_weights: Whether to load EMA weights from the checkpoint.
